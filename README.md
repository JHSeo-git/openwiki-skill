...
Write, maintain, and answer from OpenWiki wikis — repository documentation in `openwiki/` and a personal knowledge wiki in `~/.openwiki/wiki` — a port of [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki) v0.5.1 for coding agents like Claude Code and Codex.
...
- "Update the wiki" → a planner scopes the work from the git range since the last documented commit plus any page whose cited evidence moved, then one worker per page revises it; no-ops cleanly (early git check + evidence preflight) when nothing relevant changed.
