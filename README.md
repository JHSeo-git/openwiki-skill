...
Agent skills that write, maintain, and answer from OpenWiki wikis — repository documentation in `openwiki/` and a personal knowledge wiki in `~/.openwiki/wiki` — a port of [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki) v0.3.0 for coding agents like Claude Code and Codex. The wikis are OKF-compliant (Google Knowledge Catalog OKF v0.1): every concept page opens with YAML front matter (only `type` required; producer extensions preserved), pages cross-link as an evidence-backed concept graph, each directory gets a deterministically generated `index.md`, and non-compliant pages are normalized automatically at the start of every run. Pages documenting runtime flows, lifecycles, or data models embed source-grounded Mermaid diagrams, validated after every run. Wikis can be kept in any language (BCP-47): the output language persists in run metadata, index headings localize, and switching it retranslates the whole wiki. A repo-root `.openwikiignore` (gitignore-style) is a read boundary for repository runs: matching paths are never read, scanned, or documented — though a topic can still be inferred from other allowed evidence such as tests, the README, or commit messages. Since 0.3.0, repository init builds a complete wiki (no page budget): it maps the repo into a skeleton, has an independent critic subagent audit it, writes evidence-gated pages with a task-routing quickstart, then verifies the result with question-finder/answer-verifier subagents; internal links and heading anchors are validated after every run, with broken ones stamped in place for the next update to repair.
...
- "Generate documentation for this repository" → init: a skeleton-first, critic-audited, QA-verified wiki with a `openwiki/quickstart.md` task-routing entrypoint — every substantial component documented, each page with OKF front matter and concept-graph cross-links, each directory with a generated `index.md` — the upstream marker snippet (`<!-- OPENWIKI:START/END -->`) in root `AGENTS.md` and `CLAUDE.md`, and run metadata in `openwiki/.last-update.json`.
