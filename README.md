...
Agent skills that write, maintain, and answer from OpenWiki wikis — repository documentation in `openwiki/` and a personal knowledge wiki in `~/.openwiki/wiki` — a port of [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki) v0.2.5 for coding agents like Claude Code and Codex. The wikis are OKF-compliant (Google Knowledge Catalog OKF v0.1): every concept page opens with YAML front matter (only `type` required; producer extensions preserved), pages cross-link as an evidence-backed concept graph, each directory gets a deterministically generated `index.md`, and non-compliant pages are normalized automatically at the start of every run. Pages documenting runtime flows, lifecycles, or data models embed source-grounded Mermaid diagrams, validated after every run. Wikis can be kept in any language (BCP-47): the output language persists in run metadata, index headings localize, and switching it retranslates the whole wiki. A repo-root `.openwikiignore` (gitignore-style) is a read boundary for repository runs: matching paths are never read, scanned, or documented, and their git evidence is filtered out — though a topic can still be inferred from other allowed evidence such as tests, the README, or commit messages.
