...
> Reproduced from upstream `src/okf/index-labels.ts` (v0.5.2, unchanged since 0.3.0). The index pass (`skills/openwiki` Step 5, `skills/openwiki-personal` Step 4) reads this file only when the wiki's effective language is not English — for `en`, the labels are `Files` / `Directories` / `Reference` and this table is not needed. Resolution (upstream `resolveIndexLabels` / `resolveConceptTypeLabel`): full tag → primary subtag → English fallback, so `pt-BR` uses the `pt` row while `pt-PT` has its own row; an unlisted language keeps English. The `Files` / `Directories` headings are curated structural chrome, not translated prose — use the table verbatim, never your own translation. The Derived type column is the localized `type` that Step 2's normalization (re-applied while indexing) stamps on repaired pages; a language upstream leaves out of the map falls back to English `Reference`.
