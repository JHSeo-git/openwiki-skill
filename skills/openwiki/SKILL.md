...
- The actor is the producing host, matching upstream's host registry (`src/integrations/install/registry.ts`): `claude-code`, `codex`, `opencode`, or — since 0.5.0, #748 — `cursor`. **[adapted]** upstream's own runs stamp `openwiki/<version>`, which would misattribute this port's output.
