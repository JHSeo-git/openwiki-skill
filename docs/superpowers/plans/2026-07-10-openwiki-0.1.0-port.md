...
In fixture 1, commit everything, then act as the skill again with no user instruction. Expected: Step 0 finds the snippet identical (no write), early no-op fires, no metadata rewrite, report "already current".
