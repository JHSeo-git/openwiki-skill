...
- Pinned commit: `9c253af17f264ac2589ab6781e79e9bb5b5d1238` (2026-07-29, v0.2.4)
...
| `src/agent/utils.ts` — mode-aware git evidence, snapshot (ignores `_plan.md` since 0.2.1), no-op (interrupted runs retried since 0.2.4), metadata (`status` + `language` since 0.2.4), language inheritance (`createRunContext`), plan-file cleanup (`removeTemporaryPlanFile`) | `skills/openwiki/SKILL.md` Steps 1/2/4/5; `skills/openwiki-personal/SKILL.md` Steps 1/2/4/5 |
...
Next-sync note: past the 0.2.4 pin, upstream `d6e5fbe` (#165) adds `.openwikiignore` (exclude paths from doc runs) with heavy changes to mapped `src/agent/prompt.ts` and `src/agent/utils.ts` plus the write-boundary backend (`docs-only-backend.ts`) and a new `src/agent/openwiki-ignore.ts` — a real prompt/behavior port, not plumbing. Port it when syncing past v0.2.4.
