...
Agent skills that write, maintain, and answer from OpenWiki wikis — a port of [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki) v0.2.0 for coding agents such as Claude Code and Codex. Upstream 0.1.0 split into two modes, and the port mirrors that: **code mode** documents a repository in `openwiki/`; **personal mode** maintains a local knowledge wiki at `~/.openwiki/wiki` fed by the user's own sources. Since v0.2.0 the generated wikis are OKF-compliant: every page opens with Google Knowledge Catalog (OKF) YAML front matter, pages cross-link as an evidence-backed concept graph, and each wiki directory gets a deterministically regenerated `index.md`.
...
| [`skills/openwiki-ask/SKILL.md`](../skills/openwiki-ask/SKILL.md) | Q&A over both wikis, wiki-first (upstream's "Wiki-first question answering" rules), citing pages and their inline source references. |
| [`skills/migrate-wiki-to-okf/SKILL.md`](../skills/migrate-wiki-to-okf/SKILL.md) | One-time OKF migration for wikis last written before v0.2.0: add or correct front matter across every page (one subagent per directory) without changing bodies, wrapped in the matching wiki skill's snapshot/index-sync/metadata steps. |
...
3. **The system prompt** — upstream `src/agent/prompt.ts` with the `repository` output configuration inlined, reproduced *verbatim*; harness differences marked `**[adapted]**`, content owned by the other skills marked `**[omitted]**`. Since v0.2.0 the prompt carries the OKF sections: front matter requirements (five-field schema, `type` required, with the write-time validator ported as a self-check) and relationship modeling.
4. **Index sync** — regenerate every wiki directory's `index.md` deterministically (ported from `src/agent/index-middleware.ts`: front-mattered `# Files`/`# Directories` listings, write-only-on-diff); a page still lacking front matter routes through the one-time [`migrate-wiki-to-okf`](../skills/migrate-wiki-to-okf/SKILL.md) pass first.
...
- **Step 3 of both wiki skills is line-mapped to upstream `prompt.ts`** (each to its output-mode branch). Keep upstream sentences verbatim; mark every deviation `**[adapted]**` and every omission `**[omitted]**`. Three spans must stay byte-identical to upstream: the AGENTS.md snippet in `skills/openwiki/SKILL.md` Step 0 (vs `src/code-mode.ts`), the synthesis/policy blocks in the personal skill and `sources.md` (vs `prompt.ts`/`ingestion.ts`), and the `migrate-wiki-to-okf` skill body outside its `[adapted]` additions (vs upstream `skills/`) — silent unmarked edits break future syncs.
